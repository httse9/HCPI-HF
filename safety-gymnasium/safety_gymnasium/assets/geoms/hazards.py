...
class Hazards(Geom):  # pylint: disable=too-many-instance-attributes
    ...
    placements: list = field(default_factory=lambda: [(-1, -1, 1, 1)])

    # locations: list = field(default_factory=list)  # Fixed locations to override placements
    locations: list = field(default_factory=lambda: [[0.3, 0.3], [-0.3, -0.3], [0.3, -0.3], [-0.3, 0.3]])  # Fixed locations to override placements
    ...
    def cal_cost(self):
        ...
        if not self.is_constrained:
            self.last_cost_hazards = [0 for _ in range(self.num)]
            return cost
        cost['cost_hazards'] = 0
        last_cost_hazards = []
        for h_pos in self.pos:
            ...
            if h_dist <= self.size:
                ...
                last_cost_hazards.append(1)
            else:
                last_cost_hazards.append(0)
        self.last_cost_hazards = last_cost_hazards
